...
**本番はこちらで動いています → https://momochari-ramen.vercel.app**
（`main` にマージすると自動でデプロイされます）

左側のサイドメニューから各画面に移動できます（スマホでは画面上部に横並びになります）。

| URL             | 中身                                                            | 状態         |
| --------------- | --------------------------------------------------------------- | ------------ |
| `/`             | 地図ページ。岡山駅中心の地図を表示                              | 地図だけ表示 |
| `/shops`        | ラーメン店の一覧。**Supabase からデータを取って表示するお手本** | 実装済み     |
...
1. `app/<URL名>/page.tsx` を作る（例: `app/ranking/page.tsx` → `/ranking`）
2. `components/Sidebar.tsx` の `MENU` に 1 行足す

これだけでサイドメニューに出て、遷移できるようになります。
...
```
app/
  layout.tsx            # 全ページ共通の枠。サイドメニュー + 中身の 2 分割
  page.tsx              # 地図ページ（/）
  shops/page.tsx        # 店一覧（/shops）。データ取得のお手本
  measurements/page.tsx # ラーメン計測（/measurements）。枠だけ
  rides/page.tsx        # 走行データ（/rides）。枠だけ
  api/ingest/route.ts   # 走行データの受け口（IoTデバイスがここに送る）
  api/ramen/route.ts    # ラーメン計測の受け口
components/
  Sidebar.tsx           # サイドメニュー。画面を増やしたらここに 1 行足す
